...
1. Two random floats, one for choosing a random tile value, and the other for choosing a random tile location.
...
Every record type comes with a constructor function. We define a signal of the user controls by feeding ``userDirection`` to the constructor for ``Controls``:

{% highlight haskell %}
controls : Signal Controls -- construct a signal of the user controls
controls = Controls <~ userDirection
{% endhighlight %}

You may wonder why we do not just use ``userDirection`` as the user control signal. The answer is that we want to be able to extend the capabilities of the project in the future, and the separation of ``userDirection`` from ``controls`` gives us the room to define new sources of user control.
